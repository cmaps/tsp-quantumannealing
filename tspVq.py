# Script by Carla Silva and Inês Dutra 2021 :: TSP Quantum 
...
def plot_city(cities, n, sol = {}):
    ...
    nx.draw_networkx_edge_labels(G,pos,label_pos=0.2, edge_labels=labels,font_color='blue')
    plt.axis("off")
    ...
    nx.draw_networkx_edge_labels(G,pos,label_pos=0.5, edge_labels=labels,font_color='green')
    plt.axis("off")
...
#    B = float(sys.argv[3]) # Beta
    ...
    f = open('tspResults_'+'n'+str(n)+'A'+str(A)+'B'+str(B)+'.txt', 'w')
    sys.stdout = f
    ...
    sampler = EmbeddingComposite(DWaveSampler(endpoint='https://cloud.dwavesys.com/sapi', token='', solver={'topology__type': 'pegasus'}))
    ...
    response = sampler.sample_qubo(qubo, num_reads = nr, chain_strength = c, answer_mode = "raw") # n=7 c+1000, n=8 c+10000
